...
const CheckOut = () => {
    ...
    const handleSubmit = (e) => {
        ...
        const productDetails = cartItems
            .map((item) => `- ${item.name} (Size: ${item.size}, Qty: ${item.quantity}, Price: L.E ${item.price})`)
            .join('\n');
    ...
    return (
        <div className="container mx-auto p-6" role="main">
            ...
            <form onSubmit={handleSubmit} className="grid gap-6" aria-labelledby="checkout-form">
                ...
                <button
                    type="submit"
                    className="w-full text-center py-3 mt-4 font-medium text-white bg-black 
            ...
            {status === 'success' && (
                <p className="text-green-500 mt-4" role="alert">Order submitted successfully!</p>
            )}
            {status === 'failed' && (
                ...
                {cartItems.map((item, index) => (
                    <div key={index} className="mb-2">
                        <p className="text-black">
                            {item.name} (Size: {item.size}, Qty: {item.quantity}) - L.E{' '}
